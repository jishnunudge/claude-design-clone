...
<p align="center"><sub>
  ...
  👉 <a href="https://www.huasheng.ai/huashu-design-hero/">Visit interactive HTML version with SFX</a> ·
...
HTML decks for browser presentation · `html2pptx.js` reads DOM computed styles and translates each element into real PowerPoint objects · exports are **actual text frames**, double-click to edit in PPT.
...
### "Let's Talk Skill" · PM after-party presentation deck

> **Live demo · [https://skill-huasheng.vercel.app](https://skill-huasheng.vercel.app)**

A 13-page HTML deck, **built entirely with huashu-design**:

- Black-background minimal serif visual system (cover / about / hook / what / why / closing)
- 2 × 22-second cinematic demos with BGM + SFX (Nuwa skill workflow + Darwin skill workflow), each using a **completely independent visual language**:
...
- Each cinematic defaults to showing the **complete static workflow dashboard** (audience can see how the skill runs at any time); press ▶ to trigger the animation, auto-fades back to dashboard when done
- Embedded huasheng.ai 25-second hero animation (iframe with local fallback)
- Real data: 14,495 stargazers real curve (fetched via GitHub API) + DeepSeek V4 real specs (verified via WebSearch)
- Real AI assets: ran `huashu-gpt-image` for a 4×2 grid image, `extract_grid.py` cut out 8 individual transparent PNGs, used for 3D orbit float

**Slides worth referencing**:
- `/slides/slide-04b-nuwa-flow.html` · static dashboard + cinematic overlay dual-layer architecture
- `/slides/slide-06b-darwin-flow.html` · independent visual language contrast case
...
| 2 · Search official brand page | `<brand>.com/brand` · `brand.<brand>.com` · `<brand>.com/press` | Find authoritative colors |
| 3 · Download assets | SVG file → full homepage HTML → screenshot color extraction | Three fallback paths, move to next if previous fails |
| 4 · Extract colors | Grep all `#xxxxxx` from assets, sort by frequency, filter out black/white/grey | **Never guess brand colors from memory** |
| 5 · Freeze to spec | Write `brand-spec.md` + CSS variables, all HTML uses `var(--brand-*)` | Un-frozen knowledge evaporates |
...
Avoid the visual common denominator of AI output (purple gradients / emoji icons / rounded-corner + left border accent / SVG humans / Inter-as-display). Use `text-wrap: pretty` + CSS Grid + carefully chosen serif display faces + oklch colors.
...
| Agent compatibility | Claude.ai only | Any skill-compatible agent |
...
│   ├── html2pptx.js
│   └── verify.py
└── demos/                   # 9 capability demos (c*/w*), English GIF/MP4/HTML + hero v10
```
