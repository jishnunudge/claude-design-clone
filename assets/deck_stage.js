 ...
 * - Speaker notes postMessage (supports external renderer)
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render shadow DOM first (independent of child nodes, not affected by parser timing)
      this._render();

      // Guard: if script is in <head> (rather than after </deck-stage>),
      // the parser may not have processed child <section> elements yet, so
      // querySelectorAll would return empty. Defer to next event loop tick
      // to ensure all child nodes have been parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document still parsing — wait for DOMContentLoaded to catch all sections in one shot
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document already parsed (script at bottom of body or deferred) — collect next frame
